...
const addMemberCommands = {
  verifyElements: function () {
    return this.waitForElementVisible('@firstNameInput', 5000, 'First Name input is visible').verify.visible('@addPhotoButton', 'Add photo button is visible').verify.visible('@firstNameInput', 'First name input is visible').verify.visible('@lastNameInput', 'Last name input is visible').verify.visible('@usernameInput', 'Username input is visible').verify.visible('@permissionsDropdown', 'Permissions dropdown is visible').verify.visible('@addMemberButton', 'Add member button is visible');
  },
  ...
  testValidators: function () {
    return this.waitForElementVisible('@nullFirstNameValidator', 5000, 'Null first name validator visible').verify.visible('@nullLastNameValidator', 'Null last name validator visible').verify.visible('@nullUsernameValidator', 'Null username validator visible').verify.visible('@nullLocationValidator', 'Null location validator visible');
  },

  setRandomUsername: function (name) {
    let randoNum = Math.ceil(Math.random() * 1000);

    return this.waitForElementVisible('@usernameInput', 5000, 'Username input is visible').clearValue('@usernameInput').setValue('@usernameInput', name + randoNum);
  },

  selectFirstLocation: function () {
    return this.waitForElementVisible('@locationDropdown', 5000, 'Location dropdown is visible').click('@locationDropdown').waitForElementPresent('@firstLocationInDropdown', 5000, 'First location is visible').click('@firstLocationInDropdown');
  },

  fillInFirstNameInput: function (name) {
    return this.waitForElementVisible('@firstNameInput', 5000, 'First name input is visible').setValue('@firstNameInput', name);
  },

  fillInLastNameInput: function (name) {
    return this.waitForElementVisible('@lastNameInput', 5000, 'Last name input is visible').setValue('@lastNameInput', name);
  }
};
...
exports.default = AddMemberPopupPage;