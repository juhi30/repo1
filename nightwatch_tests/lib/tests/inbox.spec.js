...
module.exports = {
  ...
  'Click New Message button to show Search input': function (client) {
    ...
    inbox.newMessageClick().validateNewMessageInput().newMessageClick();
  ...
  'No search results with one character input': function (client) {
    ...
    inbox.newMessageClick().fillInNewMessageInput('f').searchResultNotVisible().newMessageClick();
  },

  'No search results with two characters input': function (client) {
    const inbox = client.page.InboxPage();

    inbox.newMessageClick().fillInNewMessageInput('fr').searchResultNotVisible().newMessageClick();
