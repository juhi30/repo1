...
const templatesCommands = {
  ...
  renderPageElements: function () {
    return this.waitForElementVisible('@createTemplateButton', 2000, 'Create template button is visible').verify.visible('@hIPAATemplate', 'HIPAA template is visible').verify.visible('@firstTestTemplate', 'First test template is visible');
  },

  validateSMSFilter: function () {
    return this.verify.visible('@filterDropdown', 'Channel filter is visible').click('@filterDropdown').waitForElementVisible('@filterTextingChannel', 5000, 'Filter choices are visible').click('@filterTextingChannel').waitForElementPresent('@filterDropdown', 5000, 'Dropdown choices are closed').verify.containsText('@filterDropdown', 'Texting', 'Texting filter is active');
  },

  validateChannelFilter: function () {
    return this.click('@filterDropdown').waitForElementVisible('@filterAll', 5000, 'All channel filter is visible').click('@filterAll')
  ...
  clickCreateTemplate: function () {
    return this.click('@createTemplateButton').waitForElementVisible('@createTemplateTitle', 2000, 'Create Template Popup is visible');
  },

  validateCreateTemplatePopup: function () {
    return this.waitForElementVisible('@createTemplateTitle', 5000, 'Title input is visible').verify.visible('@createTemplateMessage', 'Message input is visible').verify.visible('@createTemplateSaveButton', 'Create button is visible').verify.visible('@uploadFileButton', 'Upload File button is visible')
    // .verify.visible('@cancelCreateButton', 'Cancel button (X) is visible') no good xpaths for svg close button
    .click('@createTemplateSaveButton').verify.visible('@nullTemplateTitle', 'Title validator is visible').verify.visible('@nullTemplateMessage', 'Message validator is visible');
  ...
  fillOutNewTemplate: function (title, message, pathToFile) {
    return this.setValue('@createTemplateTitle', title).setValue('@createTemplateMessage', message).setValue('input[type="file"]', require('path').resolve(pathToFile)).waitForElementVisible('@uploadedFile', 5000, 'Uploaded file is visible');
  },

  saveNewTemplate: function () {
    return this.waitForElementVisible('@createTemplateSaveButton', 5000, 'Save template is visible').click('@createTemplateSaveButton').waitForElementNotPresent('@createTemplateTitle', 5000, 'Create template popup is hidden');
  },

  editTemplate: function () {
    return this.click('@firstTemplateEdit').waitForElementVisible('@createTemplateTitle', 5000, 'Edit template popup is visible').verify.visible('@firstTemplateEditSaveButton', 'Save template button is visible').setValue('@createTemplateTitle', '* added from edit popup').click('@firstTemplateEditSaveButton').waitForElementNotPresent('@firstTemplateEditSaveButton', 5000, 'Edit template popup is not present');
  },

  deleteTemplate: function () {
    return this.waitForElementVisible('@firstTemplateDelete', 1000, 'Template delete button is visible').click('@firstTemplateDelete').waitForElementVisible('@firstTemplateDeleteFinal', 1500, 'Delete template popup is visible').click('@firstTemplateDeleteFinal').waitForElementNotVisible('@firstTemplateDeleteFinal', 1500, 'Delete template popup is hidden');
  }
};
